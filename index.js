...
async function callWakaTime() {
    ...
    try {
        ...
        const content_ur = (await github.repos.getContent({ owner: 'amal910', repo: 'amal910-bot', path: 'lib/template.svg' }))
        ...
        await github.repos.createOrUpdateFileContents({ content: new_readme, owner: 'amal910', message: 'Updated status', path: 'lib/template.svg', repo: 'amal910-bot', sha: content_ur.data.sha })
    }
    catch (error) {
        console.log(error);
